Alpine.store('fileListing', { selectedFilesCount: 0, element: null });
Alpine.data('fileListing', () => ({
  lastSaveArchiver: null,
  rowClasses: {},

  init() {
    this.$store.fileListing.element = this.$el;
    document.addEventListener('files_changed', (e) => {
      this.rowClasses = {};
      for (const file of e.detail.files) {
        if (file.status === 'untracked') {
          this.rowClasses[file.file] = 'first-untracked';
          break;
        }
      }
    });
    this.$watch('$store.fileListing.selectedFilesCount', value => {
      ...
      if (selectAllCheckbox) {
        selectAllCheckbox.indeterminate = value > 0 && value < this.currentFilesCount;
        selectAllCheckbox.checked = this.currentFilesCount > 0 && value >= this.currentFilesCount;
      }
    });
    ...
    window.electronAPI.onMenuClick('git:checkout', (event, path, file, ...files) => {
      ...
      if(window.confirm(confirmMsg)){
        window.gitAPI.checkout(path, file, ...files)
          ...
          .catch(err => this.showError(textToHtml(err.message)));
      }
    });
    ...
    window.electronAPI.onMenuClick('git:add', (event, path, ...files) => {
      window.gitAPI.add(path, files)
        .then(() => this.refresh())
        .catch(err => this.showError(textToHtml(err.message)));
    });
      ...
      window.electronAPI.showSaveDialog({
        ...
        if(filePath){
          ...
          window.electronAPI.createArchiver(filePath, path, files, {zlib: {level: 6}}).then(() => {
            ...
            this.showError('保存压缩文件失败: ' + textToHtml(error.message));
          });
        }
      }).catch((error) => {
        this.showError('保存压缩文件失败: ' + textToHtml(error.message));
    ...
    window.electronAPI.onMenuClick('ignoreFiles', (event, ...files) => this.addIgnoreFile(...files));
    window.electronAPI.onMenuClick('git:fileLogs', (event, path, file) => window.gitAPI.showLogHistories(path, Alpine.store('projectPath').currentBranch, undefined, file));
    ...
    window.electronAPI.onMenuClick('ftp:upload', (event, path, ...files) => {
      window.electronAPI.ftpUploadFile(path, files, 'ftp:progress').then(succeedCount => {
        ...
        this.showError('文件上传失败: ' + textToHtml(error.message));
      })
      .finally(() => {
  ...
  showDiff(file) {
    ...
    if(!isArray && file.status!=='modified'){
    ...
    const unified = !isArray || file.length===1 ? '50' : '10'; // 如果是单个文件，默认展示50行，否则10行
    window.gitAPI.diff(projectPath, isArray ? ['HEAD', '--unified='+unified, '--', ...file] : ['HEAD', '--unified='+unified, '--', file.file], {config: ['core.quotepath=false']})
      .then(result => window.gitAPI.showDiff(projectPath, isArray ? file : [file.file], result))
      .catch(error => this.showError(textToHtml(error.message)))
  ...
  clickRow(event, tr) {
    ...
    if (checkbox) {
      ...
      if (isCheckbox) {
        ...
        this.setSelectedFileCache(checkbox.getAttribute('data-file'), checkbox.checked ? true : false);
  ...
  rowClass(file) {
    let cls = file.status;
    if (this.rowClasses[file.file]) {
      cls += ' ' + this.rowClasses[file.file];
    }
    return cls;
  },
  ...
  showPathContextMenu(currentFile) {
    ...
    menus.push(
      ...
      {label: this.isIgnoreMode ? '取消隐藏该文件' : '隐藏该文件', text: 'ignoreFiles', handler: 'ignoreFiles', args: [currentFile.file]},
    );

    if (currentFile.status === 'unmerged') {
      menus.push(
        {label: '标记为已解决', text: currentFile.file, handler: 'git:add', args: [projectPath, currentFile.file]}
      );
    }
    if (currentFile.status !== 'untracked' && currentFile.status !== 'unmerged') {
      menus.push(
        {label: '放弃修改', text: currentFile.file, handler: 'git:checkout', args: [projectPath, currentFile.file]}
      );
    }
    if (currentFile.status !== 'untracked' && currentFile.status !== 'added') {
      menus.push(
        {label: '提交历史', text: currentFile.file, handler: 'git:fileLogs', args: [projectPath, currentFile.file]}
      );
    }
    ...
    if(files.length>0){
      ...
      menus.push(
        ...
        {label: this.isIgnoreMode ? '取消隐藏选中的文件' : '隐藏选中的文件', text: 'ignoreFiles', handler: 'ignoreFiles', args: paths}
      );

      if (unmergedFiles.length > 0) {
        menus.push(
          {label: '标记选中文件为已解决', text: 'gitAdd', handler: 'git:add', args: [projectPath, ...unmergedFiles]}
        );
      }
      if (modifiedFiles.length > 0) {
        menus.push(
          {label: '查看选中文件的差异', text: 'gitDiff', handler: 'showFileDiff', args: [modifiedFiles]},
          {label: '放弃选中文件的修改', text: 'checkout', handler: 'git:checkout', args: [projectPath, ...modifiedFiles]}
        );
      }
  ...
  addIgnoreFile(...file) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (path) {
        ...
        allIgnoreFiles[path] = Array.from(this.ignoreFiles).join(';');
        window.electronStore.setJSON('ignoreFiles', JSON.stringify(allIgnoreFiles));
        resolve(true);
      } else {
